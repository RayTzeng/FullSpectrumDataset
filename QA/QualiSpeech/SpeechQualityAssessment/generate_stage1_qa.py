...
# Priority list used when --target-field is not provided.
...
def build_context(
    ...
    # Field-specific helpers, e.g. mos_rounded, mos_1dp, mos_2dp
...
def weighted_sample_without_replacement(
    ...
    Larger weights lead to higher inclusion probability.
    """
    if k <= 0:
        return []
    if k > len(weights):
...
def choose_template_indices(
    ...
    # If user asks for more samples than number of templates:
    # first cover all templates once (weighted ordering), then sample extras with replacement.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample weighted QA templates and instantiate them against metadata. "
            "Supports .jsonl and .jsonl.gz input/output."
        )
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Optional explicit target field. Recommended when metadata has more than one "
            "numeric field, e.g. --target-field mos"
        ),
    )
