...
def weighted_sample_without_replacement(
    ...
    Larger weights -> higher inclusion probability.
    """
    if k <= 0:
        return []
    if k > len(weights):
...
def choose_template_indices(
    ...
    # Cover all templates once first, then continue with replacement.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample weighted Stage-2 templates and instantiate them against metadata. "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Optional explicit target field. Recommended when metadata has multiple "
            "numeric fields, e.g. RT60."
        ),
    )
