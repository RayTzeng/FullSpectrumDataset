...
def infer_target_field(metadata_row: Dict[str, Any], target_field: Optional[str]) -> str:
...
def build_context(
    ...
    # Common aliases / formatted variants
...
def get_template_weight(template_obj: Dict[str, Any], index: int) -> float:
    ...
    if not (0.0 < weight <= 1.0):
        raise ValueError(
            f"Template row {index} has invalid weight {weight}. "
...
def weighted_sample_without_replacement(
    ...
    Larger weights lead to higher selection probability.
    """
    if k <= 0:
        return []
    if k > len(weights):
...
def choose_template_indices(
    ...
    # First cover all templates without replacement, then continue with replacement.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample weighted Stage-1 templates and instantiate them against metadata. "
    ...
    parser.add_argument("--template-jsonl", required=True, type=str,
                        help="Path to template JSONL or JSONL.GZ")
    ...
    parser.add_argument("--samples-per-entry", type=int, default=1,
                        help="How many QA pairs to generate per metadata row")
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Optional explicit target field. Recommended for datasets with "
            "multiple numeric fields, e.g. RT60."
        ),
    )
