...
# Compatibility for a few Stage-2 answer-template styles that mix braced expressions
# with inline Python ternary logic outside braces.
MIXED_TERNARY_TWO_BRANCHES = re.compile(
    r"^\{(?P<expr1>[^{}]+)\}\s*(?P<suffix1>.*?)\s+if\s+(?P<cond1>.+?)\s+else\s+"
    r"(?P<expr2>.+?)\s*\+\s*'(?P<suffix2>[^']*)'\s+if\s+(?P<cond2>.+?)\s+else\s+'(?P<false>[^']*)'$"
)
MIXED_TERNARY_ONE_EXPR_TWO_FALSES = re.compile(
    r"^\{(?P<expr1>[^{}]+)\}\s*(?P<suffix1>.*?)\s+if\s+(?P<cond1>.+?)\s+else\s+"
    r"'(?P<false1>[^']*)'\s+if\s+(?P<cond2>.+?)\s+else\s+'(?P<false2>[^']*)'$"
)
...
def eval_expr(expr: str, context: Dict[str, Any]) -> Any:
    try:
        return eval(expr, get_safe_globals(), context)
...
def try_render_mixed_ternary(template: str, context: Dict[str, Any]) -> Optional[str]:
    """
    Compatibility for a small number of Stage-2 answer templates like:
      {format_fixed(lower - C50, 2)} dB if C50 < lower else format_fixed(C50 - upper, 2) + ' dB' if C50 >= upper else '0.00 dB'

    These are not standard braced templates, but we can still interpret them safely.
    """
    m = MIXED_TERNARY_TWO_BRANCHES.match(template)
    if m:
        expr1 = m.group("expr1").strip()
        suffix1 = m.group("suffix1")
        cond1 = m.group("cond1").strip()
        expr2 = m.group("expr2").strip()
        suffix2 = m.group("suffix2")
        cond2 = m.group("cond2").strip()
        false_value = m.group("false")

        if bool(eval_expr(cond1, context)):
            return f"{eval_expr(expr1, context)}{suffix1}"
        if bool(eval_expr(cond2, context)):
            return f"{eval_expr(expr2, context)}{suffix2}"
        return false_value

    m = MIXED_TERNARY_ONE_EXPR_TWO_FALSES.match(template)
    if m:
        expr1 = m.group("expr1").strip()
        suffix1 = m.group("suffix1")
        cond1 = m.group("cond1").strip()
        false1 = m.group("false1")
        cond2 = m.group("cond2").strip()
        false2 = m.group("false2")
...
def render_template(template: str, context: Dict[str, Any]) -> str:
    # First, handle the special mixed conditional answer-template style if present.
    mixed = try_render_mixed_ternary(template, context)
...
def weighted_sample_without_replacement(
    ...
    Larger weights give higher inclusion probability.
    """
    if k <= 0:
        return []
    if k > len(weights):
        raise ValueError("k cannot be larger than the number of templates")
...
def choose_template_indices(
    ...
    extra = rng.choices(population=population, weights=weights, k=samples_per_entry - num_templates)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample weighted Stage-2 question/answer templates and instantiate them against metadata. "
            "Outputs JSONL or JSONL.GZ rows with keys: question, answer, metadata."
        )
    )
    parser.add_argument("--template-jsonl", required=True, type=str, help="Path to weighted Stage-2 template JSONL")
    parser.add_argument("--metadata", required=True, type=str, help="Path to metadata JSONL or JSONL.GZ")
    parser.add_argument("--output", required=True, type=str, help="Path to output JSONL or JSONL.GZ")
    parser.add_argument("--samples-per-entry", required=True, type=int, help="Number of QA pairs to sample per metadata row")
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Optional explicit target field. Recommended when metadata contains "
            "multiple numeric fields, e.g. --target-field C50"
        ),
    )
