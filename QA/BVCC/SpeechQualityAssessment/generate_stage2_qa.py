...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample weighted Stage-2 question/answer templates and instantiate "
            "them against metadata. Supports .jsonl and .jsonl.gz input/output."
        )
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Optional explicit target field. Recommended when metadata has more than "
            "one numeric field, e.g. --target-field mos"
        ),
    )
