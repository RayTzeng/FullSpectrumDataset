#!/usr/bin/env python3
"""
Generate Multi-Talker ASR QA pairs from a weighted template JSONL and a metadata JSONL(.gz).

Target field: `transcript` -- speaker-attributed lines of the form "SpeakerNN: text",
one line per speaker turn.  Answer expressions may be the bare field or one of the
deterministic formatters defined below.
...
python generate_qa.py \
  ...
  --metadata /home/tseng/FullSpectrumDataset/metadata/AMI/MultiTalkerASR/train.jsonl.gz \
...
TURN_RE = re.compile(r"^(Speaker\d+):[ \t]?(.*)$", re.S)


def _split_turns(transcript: Any) -> List[tuple]:
    """Split a speaker-attributed transcript into (speaker_label_or_None, text) turns."""
    turns = []
    for line in str(transcript).split("\n"):
        m = TURN_RE.match(line)
        turns.append((m.group(1), m.group(2)) if m else (None, line))
    return turns


def _first_appearance_index(turns: List[tuple]) -> Dict[str, int]:
    """Map each speaker label to its 0-based order of first appearance."""
    seen: List[str] = []
    for spk, _ in turns:
...
def strip_speaker_labels(transcript: Any) -> str:
    """Drop the 'SpeakerNN: ' prefix from every turn, keeping one turn per line."""
    return "\n".join(text for _, text in _split_turns(transcript))


def flatten_transcript(transcript: Any) -> str:
    """Speaker labels removed and all turns joined into a single whitespace-normalised line."""
    return re.sub(r"\s+", " ", strip_speaker_labels(transcript)).strip()


def relabel_speakers(transcript: Any, style: str) -> str:
    """Renumber speakers by order of first appearance and re-prefix each turn.

    style: 'numbered' -> 'Speaker 1: '   'short' -> 'S1: '   'letter' -> 'A: '
    """
    turns = _split_turns(transcript)
    index = _first_appearance_index(turns)
    out = []
    for spk, text in turns:
        ...
        if style == "numbered":
            ...
            raise ValueError(f"Unknown relabel style: {style!r}")
        out.append(f"{label}: {text}" if text else f"{label}:")
    return "\n".join(out)


def count_speakers(transcript: Any) -> int:
    """Number of distinct speaker labels present in the transcript itself.

    Deliberately not len(speakers): on NotSoFar the `speakers` field can list names
    that never appear in a given segment's transcript, which would make a cascade
    answer contradict the transcript printed beneath it.
    """
    return len(_first_appearance_index(_split_turns(transcript)))


def count_cascade(transcript: Any) -> str:
    """'There are N speakers.' followed by a blank line and the attributed transcript."""
    n = count_speakers(transcript)
    lead = "There is 1 speaker." if n == 1 else f"There are {n} speakers."
    return f"{lead}\n\n{transcript}"


def count_cascade_compact(transcript: Any) -> str:
    """'Speakers: N' header line followed by the attributed transcript."""
    return f"Speakers: {count_speakers(transcript)}\n{transcript}"


SAFE_GLOBALS = {
    "__builtins__": {},
    "strip_speaker_labels": strip_speaker_labels,
    "flatten_transcript": flatten_transcript,
    "relabel_speakers": relabel_speakers,
    "count_speakers": count_speakers,
    "count_cascade": count_cascade,
    "count_cascade_compact": count_cascade_compact,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate Multi-Talker ASR QA pairs from templates and metadata.",
    ...
    p.add_argument("--target-field", default="transcript",
