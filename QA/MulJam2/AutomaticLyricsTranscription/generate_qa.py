...
Input:
  1. template.jsonl
     Each line should contain either:
       {"question_template": "...", "answer_template": "lyrics", "weight": 0.9}
     or:
       {"question": "...", "answer": "lyrics", "weight": 0.9}
...
def build_question(template: str, metadata: Dict[str, Any]) -> str:
    ...
    lang_display defaults to metadata['language'].
    """
    fmt = dict(metadata)
    if "lang_display" not in fmt:
        fmt["lang_display"] = metadata.get('language', "")
...
def generate_qa_for_entry(
    ...
    if "lyrics" not in metadata:
        raise ValueError(f"Metadata entry is missing required field 'lyrics': {metadata}")
