...
Output format (gzipped JSONL):
    {"question": ..., "answer": ..., "metadata": {...}}

Expected template format:
    {"template_id": "...", "question": "...", "answer": "...", "weight": 0.83}

Notes
-----
- `question` is treated as a Python format string. Placeholders like {text}, {id}, etc.
  are filled from the metadata entry.
- `answer` is evaluated using a restricted expression evaluator.
  Example:
      "caption"
      "caption.lower()"
      "'Speaker: ' + text"
- `weight` controls weighted template sampling.
- By default, templates are sampled WITHOUT replacement for each metadata entry.
"""
...
class SafeEvaluator:
    """Restricted expression evaluator for template answers."""
...
def count_jsonl_entries(path: str) -> int:
    count = 0
    with open_text_auto(path) as f:
        for line in f:
            if line.strip():
...
def validate_templates(templates: Sequence[Dict[str, Any]]) -> List[float]:
    ...
    if not any(w > 0 for w in weights):
        raise ValueError("No templates have positive weight.")
...
def generate(
    ...
    total_metadata = count_jsonl_entries(metadata_path)
