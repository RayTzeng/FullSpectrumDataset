#!/usr/bin/env python3
"""
Generate QA pairs for TinyStress / Stressed Speech ASR from a template .jsonl
file and a metadata .jsonl/.jsonl.gz file.

Output format (one JSON object per line):
    {"question": ..., "answer": ..., "metadata": {...}}

The TinyStress `text` field stores the transcription with prosodically stressed
words wrapped in Markdown-style double asterisks, e.g.

    "One **day,** a little girl named Lily found a needle in her room."

Attached punctuation lives *inside* the markers (`**day,**`, `**"Yes,**`), and a
single utterance may carry several stressed words.

Supported answer expressions
----------------------------
    text                              transcript with the original ** markers
    stressed_words(text)              "little, girl"   (punctuation stripped)
    stressed_words_bracketed(text)    "[little, girl]"
    mark_upper(text)                  stressed words in ALL CAPS
    mark_brackets(text)               stressed words in [square brackets]
    mark_em(text)                     stressed words in <em>...</em>
    mark_single_star(text)            stressed words in *single asterisks*
    plain_transcript(text)            markers removed
    cascade_plain_then_stress(text)   "Transcription: ...\nStressed words: ..."
    cascade_marked_then_stress(text)  same, but the transcript keeps ** markers
...
python generate_qa.py \
  ...
  --metadata /home/tseng/FullSpectrumDataset/metadata/TinyStress/train.jsonl.gz \
...
python generate_qa.py \
  --template template.jsonl \
  --metadata /home/tseng/FullSpectrumDataset/metadata/TinyStress/test.jsonl.gz \
...
# --------------------------------------------------------------------------- #
# TinyStress-specific formatters
# --------------------------------------------------------------------------- #
...
def _stressed_tokens(text: str) -> List[str]:
    """Raw tokens inside ** ** markers, in spoken order (punctuation included)."""
    return STRESS_RE.findall(str(text))


def _strip_punct(token: str) -> str:
    """Drop leading/trailing punctuation, keeping internal apostrophes (Lily's)."""
...
def stressed_words(text: str) -> str:
    """Comma-separated stressed words, punctuation stripped, in spoken order."""
    words = [w for w in (_strip_punct(t) for t in _stressed_tokens(text)) if w]
    return ", ".join(words)


def stressed_words_bracketed(text: str) -> str:
    """Stressed words as a bracketed list: "[little, girl]"."""
    return "[" + stressed_words(text) + "]"


def mark_upper(text: str) -> str:
    """Stressed words in ALL CAPS instead of ** markers."""
    return STRESS_RE.sub(lambda m: m.group(1).upper(), str(text))


def mark_brackets(text: str) -> str:
    """Stressed words in [square brackets] instead of ** markers."""
    return STRESS_RE.sub(lambda m: "[" + m.group(1) + "]", str(text))


def mark_em(text: str) -> str:
    """Stressed words in <em>...</em> instead of ** markers."""
    return STRESS_RE.sub(lambda m: "<em>" + m.group(1) + "</em>", str(text))


def mark_single_star(text: str) -> str:
    """Stressed words in *single asterisks* instead of ** markers."""
    return STRESS_RE.sub(lambda m: "*" + m.group(1) + "*", str(text))


def cascade_plain_then_stress(text: str) -> str:
    """Unmarked transcript, then the stressed words on a second line."""
    return (
        "Transcription: "
...
def cascade_marked_then_stress(text: str) -> str:
    """Stress-marked transcript, then the stressed words on a second line."""
    return (
        "Transcription: "
        + str(text)
        + "\nStressed words: "
...
TINYSTRESS_HELPERS = {
    ...
    "stressed_words_bracketed": stressed_words_bracketed,
    "mark_upper": mark_upper,
    "mark_brackets": mark_brackets,
    "mark_em": mark_em,
    "mark_single_star": mark_single_star,
    "cascade_plain_then_stress": cascade_plain_then_stress,
    "cascade_marked_then_stress": cascade_marked_then_stress,
}
...
class SafeEvaluator:
    ...
    def _eval_node(self, node: ast.AST) -> Any:
        ...
        if isinstance(node, ast.Name):
            ...
            if node.id in TINYSTRESS_HELPERS:
                return TINYSTRESS_HELPERS[node.id]
...
def render_answer(answer_expr: Any, metadata: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available helpers: {sorted(TINYSTRESS_HELPERS)}"
        ) from e
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate TinyStress Stressed-Speech-ASR QA JSONL from template JSONL and metadata JSONL(.gz)."
    )
